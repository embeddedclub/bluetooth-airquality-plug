                                              ...
                                              xensiv_pasco2_mtb_interrupt_cb_t * interrupt_cb)
{
    ...
    if (XENSIV_PASCO2_OK == res)
    {
        ...
        {
#if defined(CYHAL_API_VERSION) && (CYHAL_API_VERSION >= 2)
            cyhal_gpio_register_callback(pin, (cyhal_gpio_callback_data_t*) interrupt_cb);
#else
            cyhal_gpio_register_callback(pin, interrupt_cb->cb, interrupt_cb->arg);
#endif            
            cyhal_gpio_enable_event(pin, (int_config.b.int_typ == (uint32_t)XENSIV_PASCO2_INTERRUPT_TYPE_LOW_ACTIVE) ? CYHAL_GPIO_IRQ_FALL : CYHAL_GPIO_IRQ_RISE, intr_priority, true);
        }
...
int32_t xensiv_pasco2_plat_i2c_transfer(void * ctx, uint16_t dev_addr, const uint8_t * tx_buffer, size_t tx_len, uint8_t * rx_buffer, size_t rx_len)
{
    ...
    cy_rslt_t result = cyhal_i2c_master_write(i2c, dev_addr, tx_buffer, (uint16_t)(tx_len & 0xffffU), XENSIV_PASCO2_I2C_TIMEOUT_MS, send_stop);

    if ((CY_RSLT_SUCCESS == result) && (rx_buffer != NULL))
    {
        result = cyhal_i2c_master_read(i2c, dev_addr, rx_buffer, (uint16_t)(rx_len & 0xffffU), XENSIV_PASCO2_I2C_TIMEOUT_MS, true);
    }
